<div class="card card-register">
    ...
    <div class="js_panel {if $do == 'up'}x-hidden{/if}">
        <div class="card-header">
            ...
            <h4 class="card-title" style="padding-left:20px;">{__("Login")}</h4>
        </div>
        <div class="card-body">
                            ...
                            <select name="phone_code" id="phone_code" class="form-control" required>
                                {foreach $countries as $country}
                                    {if $country['phone_code'] != ""}
                                        <option value="{$country['phone_code']}" {if $country['default']}selected{/if}>{__({$country['country_name']})} ({$country['phone_code']})</option>
                                    {/if}
                                {/foreach}
                            </select>
                            <input name="phone" type="text" class="form-control" placeholder='{__("Phone number")}' required>
                        </div>
                    </div>
                {/if}
                <div class="form-group">
