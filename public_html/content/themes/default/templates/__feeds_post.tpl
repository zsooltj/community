{if !$standalone}<li>{/if}
    ...
    <div class="post 
        ...
        <div class="post-body">
            ...
            {/if}
            {* <div>points: {$user->_data['user_points']}</div> *}
            <!-- post top alert -->
            ...
            <div class="post-stats clearfix">
                ...
                {if $post['post_type'] == "shared" && $post['origin']['post_type'] == "video"}
                    <span>
                        <i class="fa fa-eye"></i> {$post['origin']['video']['views']}
                    </span>
                {/if}
                <!-- video views -->
                ...
                {if $post['post_type'] == "shared" && $post['origin']['post_type'] == "audio"}
                    <span>
                        <i class="fa fa-eye"></i> {$post['origin']['audio']['views']}
                    </span>
                {/if}
                <!-- audio views -->
            ...
            {if $user->_logged_in && $_get != "posts_information"}
                <div class="post-actions clearfix">
                    ...
                    <!-- add one point to author -->
                    {* <div class="action-btn js_add_one" data-authorid="{$post['author_id']}" data-url="posts/pointsystem.php?do=add_one_user_point&author_id={$post['author_id']}">
                            {include file='__svg_icons.tpl' icon="upload" width="16px" height="16px" class="mr5"}
                            <span>{__("Add one point to user: ")} {$post['author_id']}</span>
                    </div> *}
