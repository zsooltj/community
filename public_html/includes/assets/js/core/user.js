...
api["pages_groups_events/delete"] =
  ajax_path + "pages_groups_events/delete.php";
/* ads */
...
function data_heartbeat() {
  ...
  if (
    ...
    data["last_post"] =
      posts_stream_staging.find("li:first .post").data("id") ||
      posts_stream.find("li:first .post").data("id") ||
      0;
    ...
    function (response) {
      if (response.callback) {
        ...
        if (response.delete_last_request == true) {
          $(".js_live-requests")
            .find(".js_scroller ul:first li:first")
            .remove();
          var requests_counter =
            parseInt($(".js_live-requests").find("span.counter").text()) - 1;
          ...
          if (response.requests) {
            if ($(".js_live-requests").find(".js_scroller ul").length > 0) {
              $(".js_live-requests")
                .find(".js_scroller ul:first")
                .prepend(response.requests);
            } else {
              $(".js_live-requests")
            ...
            var requests_counter =
              parseInt($(".js_live-requests").find("span.counter").text()) +
              response.requests_count;
            $(".js_live-requests")
              .find("span.counter")
              .text(requests_counter)
              .show();
        ...
        if (response.conversations) {
          ...
          if (response.conversations_count > 0) {
            $(".js_live-messages")
              .find("span.counter")
              .text(response.conversations_count)
              .show();
            if (chat_sound) {
              $("#chat-sound")[0].play();
            }
        ...
        if (response.notifications) {
          ...
          if ($(".js_live-notifications").find(".js_scroller ul").length > 0) {
            $(".js_live-notifications")
              .find(".js_scroller ul")
              .prepend(response.notifications);
          } else {
            $(".js_live-notifications")
              .find(".js_scroller")
          ...
          var notifications =
            parseInt($(".js_live-notifications").find("span.counter").text()) +
        ...
        if (response.posts) {
          ...
          var posts_count =
            parseInt(posts_stream_staging_btn.find("span").text()) +
...
$(function () {
  ...
  if ($(".js_wysiwyg-advanced").length > 0) {
    tinymce.init({
      ...
      extended_valid_elements:
        "script[src|async|defer|type|charset],marquee[class|width|height|align|onmouseover|onmouseout|behavior|direction|scrollamount|scrolldelay]",
      image_advtab: true,
      paste_data_images: true,
      images_upload_handler: function (blobInfo, success, failure) {
        var xhr, formData;
  ...
  $("body").on("click", ".js_view-staging-posts", function () {
    ...
    $(".js_posts_stream")
      .find("ul:first")
      .prepend($(".js_posts_stream_staging").html());
  ...
  $("body").on("click focus", ".js_autocomplete input", function () {
    ...
    if (
      $(this).val() != "" &&
      $(this).next(".dropdown-menu").find("li").length > 0
    ) {
  ...
  $("body").on("click focus", ".js_autocomplete-tags input", function () {
    /* check maximum number of tags */
    if (
      $(this).parents(".js_autocomplete-tags").find("ul.tags li").length > 9
    ) {
      return;
    }
    /* only show again if the input & dropdown-menu are not empty */
  ...
  $("body").on("click", ".js_tag-add", function () {
    ...
    var tag =
      ...
      '">' +
      name +
      '<button type="button" class="close js_tag-remove" title="' +
      __["Remove"] +
      '"><span>&times;</span></button></li>';
  ...
  $("body").on("mouseenter", ".js_user-popover", function () {
    ...
    var _timeout = setTimeout(function () {
      ...
      if ($("html").attr("dir") == "RTL") {
        ...
        if (available < 400) {
          $("body").append(
            '<div class="user-popover-wrapper" style="position: fixed; top: ' +
              posY +
              "px; left:" +
              posX +
  ...
  $("body").on("submit", ".x-uploader", function (e) {
    ...
    /* get multiple */
    var multiple =
      $(this).find(".js_x-uploader").data("multiple") !== undefined
        ? true
        : false;
    ...
    if (type == "photos") {
      ...
      if (
        ...
        for (var i = 0; i < files_num; ++i) {
          $(
            '<li class="loading"><div class="progress x-progress"><div class="progress-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div></div></li>'
          )
            .appendTo(loader)
            .show();
        }
    ...
    function _handle_success(response) {
      ...
      if (response.callback) {
        if (handle == "publisher") {
          ...
          if (
            (type == "photos" &&
              jQuery.isEmptyObject(publisher.data("photos"))) ||
            type != "photos"
          ) {
            /* hide attachments */
            attachments.hide();
        ...
        if (type == "photos") {
          ...
          if (
            ...
            if (cover_image.length == 0) {
              $(".profile-cover-wrapper").prepend(
                "<img class='js_position-cover-cropped' data-init-position='0px' src='" +
                  image_path +
                  "' />"
              );
              $(".profile-cover-wrapper").prepend(
                "<img class='js_position-cover-full x-hidden' src='" +
                  image_path +
                  "' />"
              );
            } else {
              ...
              /* remove lightbox */
              cover_image
                .removeClass("js_lightbox")
                .removeAttr("data-id")
                .removeAttr("data-image")
                .removeAttr("data-context");
            }
            ...
            for (var i in response.files) {
              ...
              attachments.find("ul").append(
                render_template("#uploader-attachments-image-item", {
                  src: response.files[i]["source"],
            ...
            var image_path = uploads_path + "/" + response.file;
            attachments.find("ul").append(
              render_template("#comment-attachments-item", {
                src: response.file,
                image_path: image_path,
              })
            );
            ...
            var image_path = uploads_path + "/" + response.file;
            attachments.find("ul").append(
              render_template("#chat-attachments-item", {
                src: response.file,
                image_path: image_path,
              })
            );
          ...
          if (handle == "publisher") {
            ...
            for (var i in response.files) {
              ...
              /* add publisher-attachments */
              attachments.find("ul").append(
                render_template("#uploader-attachments-video-item", {
  ...
  $("body").on(
    "show.bs.dropdown",
    ".js_live-requests, .js_live-messages, .js_live-notifications",
    function () {
      var _this = $(this);
  ...
  $("body").on("click", ".js_notifications-sound-toggle", function () {
    ...
    $.get(
      api["users/settings"],
      {
        ...
        notifications_sound: notifications_sound ? 1 : 0,
      },
      function (response) {
        /* check the response */
        if (!response) return;
  ...
  $("body").on("click", ".js_friend-accept, .js_friend-decline", function () {
    ...
    var _do = $(this).hasClass("js_friend-accept")
      ? "friend-accept"
      : "friend-decline";
      ...
      function (response) {
        if (response.callback) {
          ...
          if (_do == "friend-accept") {
            parent.append(
              '<button type="button" class="btn btn-sm btn-success btn-delete js_friend-remove" data-uid="' +
                id +
  ...
  $("body").on(
    "click",
    ".js_friend-add, .js_friend-cancel, .js_friend-remove",
    function () {
      var _this = $(this);
      ...
      $.post(
        ...
        function (response) {
          if (response.callback) {
            ...
            if (_do == "friend-add") {
              _this.after(
                '<button type="button" class="btn btn-sm btn-warning js_friend-cancel" data-uid="' +
                  id +
              ...
              _this.after(
                '<button type"button" class="btn btn-sm btn-success js_friend-add" data-uid="' +
                  id +
    ...
    $.post(
      ...
      function (response) {
        if (response.callback) {
          ...
          if (_do == "follow") {
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-info js_unfollow" data-uid="' +
                id +
                '"><i class="fa fa-check mr5"></i>' +
                __["Following"] +
                "</button>"
            ...
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-info js_follow" data-uid="' +
                id +
                '"><i class="fa fa-rss mr5"></i>' +
                __["Follow"] +
                "</button>"
            );
          }
  ...
  $("body").on("click", ".js_delete-user", function (e) {
    ...
    confirm(
      ...
      function () {
        $.post(
          api["users/delete"],
          { password_check: $("#modal-password-check").val() },
          function (response) {
            /* check the response */
            if (response.callback) {
              eval(response.callback);
    ...
    $.post(
      ...
      function (response) {
        if (response.callback) {
          ...
          if (_do == "page-like") {
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-primary js_unlike-page" data-id="' +
                id +
            ...
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-primary js_like-page" data-id="' +
                id +
                '"><i class="fa fa-thumbs-up mr5"></i>' +
                __["Like"] +
                "</button>"
    ...
    $.post(
      ...
      function (response) {
        if (response.callback) {
          ...
          if (_do == "page-boost") {
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-danger js_unboost-page" data-id="' +
                id +
            ...
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-danger js_boost-page" data-id="' +
                id +
                '"><i class="fa fa-bolt mr5"></i>' +
                __["Boost"] +
                "</button>"
  ...
  $("body").on(
    "click",
    ".js_page-admin-addation, .js_page-admin-remove",
    function () {
      var _this = $(this);
      var id = _this.data("id");
      var uid = _this.data("uid") || 0;
      var _do = _this.hasClass("js_page-admin-addation")
        ? "page-admin-addation"
        : "page-admin-remove";
      /* button loading */
      button_status(_this, "loading");
        ...
        function (response) {
          if (response.callback) {
            ...
            if (_this.hasClass("js_page-admin-addation")) {
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-danger js_page-admin-remove" data-id="' +
                  id +
                  '" data-uid="' +
                  uid +
              ...
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-primary js_page-admin-addation" data-id="' +
                  id +
                  '" data-uid="' +
                  uid +
                  '"><i class="fa fa-check mr5"></i>' +
      ...
      function (response) {
        if (response.callback) {
          ...
          if (_this.hasClass("js_join-group")) {
            if (privacy == "public") {
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-success btn-delete js_leave-group" data-id="' +
                  id +
                  '" data-privacy="' +
                  privacy +
                  '"><i class="fa fa-check mr5"></i>' +
              ...
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-warning js_leave-group" data-id="' +
                  id +
                  '" data-privacy="' +
                  privacy +
                  '"><i class="fa fa-clock mr5"></i>' +
            ...
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-success js_join-group" data-id="' +
                id +
                '" data-privacy="' +
                privacy +
                '"><i class="fa fa-user-plus mr5"></i>' +
  ...
  $("body").on(
    "click",
    ".js_group-request-accept, .js_group-request-decline",
    function () {
      var _this = $(this);
      var id = _this.data("id");
      var uid = _this.data("uid") || 0;
      var _do = _this.hasClass("js_group-request-accept")
        ? "group-accept"
        : "group-decline";
      /* button loading */
      button_status(_this, "loading");
  ...
  $("body").on(
    "click",
    ".js_group-admin-addation, .js_group-admin-remove",
    function () {
      var _this = $(this);
      var id = _this.data("id");
      var uid = _this.data("uid") || 0;
      var _do = _this.hasClass("js_group-admin-addation")
        ...
        function (response) {
          if (response.callback) {
            ...
            if (_this.hasClass("js_group-admin-addation")) {
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-danger js_group-admin-remove" data-id="' +
                  id +
                  '" data-uid="' +
                  uid +
                  '"><i class="fa fa-trash mr5"></i>' +
              ...
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-primary js_group-admin-addation" data-id="' +
                  id +
                  '" data-uid="' +
                  uid +
                  '"><i class="fa fa-check mr5"></i>' +
    ...
    $.post(
      ...
      function (response) {
        if (response.callback) {
          ...
          if (_do == "event-go") {
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-success js_ungo-event" data-id="' +
                id +
                '"><i class="fa fa-check mr5"></i>' +
                __["Going"] +
                "</button>"
            ...
            _this.replaceWith(
              '<button type="button" class="btn btn-sm btn-success js_go-event" data-id="' +
                id +
                '"><i class="fa fa-calendar-check mr5"></i>' +
                __["Going"] +
                "</button>"
            );
          }
  ...
  $("body").on(
    "click",
    ".js_interest-event, .js_uninterest-event",
    function () {
      var _this = $(this);
      var id = _this.data("id");
      var _do = _this.hasClass("js_interest-event")
        ? "event-interest"
        : "event-uninterest";
      /* button loading */
      button_status(_this, "loading");
        ...
        function (response) {
          if (response.callback) {
            ...
            if (_do == "event-interest") {
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-primary js_uninterest-event" data-id="' +
                  id +
                  '"><i class="fa fa-check mr5"></i>' +
                  __["Interested"] +
                  "</button>"
              ...
              _this.replaceWith(
                '<button type="button" class="btn btn-sm btn-primary js_interest-event" data-id="' +
                  id +
  ...
  $("body").on(
    "click",
    ".js_page-invite, .js_group-invite, .js_event-invite",
    function () {
      var _this = $(this);
      var id = _this.data("id");
      var uid = _this.data("uid") || 0;
  ...
  $("body").on(
    "click",
    ".js_delete-page, .js_delete-group, .js_delete-event",
    function (e) {
      e.preventDefault();
      var id = $(this).data("id");
  ...
  $("body").on(
    "click",
    ".js_ads-stop-campaign, .js_ads-resume-campaign",
    function () {
      var id = $(this).data("id");
  ...
  /* get potential reach */
  $(
    "#js_ads-audience-countries, #js_ads-audience-gender, #js_ads-audience-relationship"
  ).on("change", function () {
    /* get values */
