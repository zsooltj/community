...
class User
{
    ...
    public function get_balance($user_id){
        global $db, $system;
        $result = $db->query(sprintf("SELECT user_points FROM users WHERE user_id = %s",  secure($user_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        $return = $result->fetch_assoc()['user_points'];
        return $return;
              
    }

    public function update_user_point($type, $user_id, $amount){
        global $db, $system;
        switch ($type) {
            case 'add':
                $db->query(sprintf("UPDATE users SET user_points = user_points + %s WHERE user_id = %s", secure($amount, 'int'), secure($user_id, 'int'))) or _error("SQL_ERROR_THROWEN");
                break;
            case 'remove':
                $db->query(sprintf("UPDATE users SET user_points = user_points - %s WHERE user_id = %s", secure($amount, 'int'), secure($user_id, 'int'))) or _error("SQL_ERROR_THROWEN");
                break;
